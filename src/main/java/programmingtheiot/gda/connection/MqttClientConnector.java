 ...
 */ 
 ...
 /**
  * Mqtt client connector class that can interact with MQTT broker
  * 
  */
 public class MqttClientConnector implements IPubSubClient, MqttCallbackExtended
 {
	 ...
	 /**
	  * Constructor to initialize the properties needed by MQTT client
	  * 
	  */
	 ...
	 //Method to connect client to the broker
	 @Override
	 ...
	 //Method to handle all publish functionality
	 protected boolean publishMessage(String topicName, byte[] payload, int qos)
	{
	 ...
	 // Method to handle all subscribe functionality
	...
	protected boolean subscribeToTopic(String topicName, int qos, IMqttMessageListener listener)
	{
	 ...
	 // Method to handle unsubscribe functionality
	...
	public boolean unsubscribeFromTopic(ResourceNameEnum topicName)
	{
	 ...
	 // callbacks
	 //Callbacks to handle connection notifications
	 @Override
	 ...
	 //Callback method to handle publish notifications
	 @Override
	 ...
	 //Callback method to be invoked whenever a message is received on the topic for which the client has 
	 //subscribed 
	 @Override
	 ...
	 // Callback class for actuator data response messages subscription
	 ...
	 // Callback class for sensor data response messages subscription
	 private class SensorDataMessageListener implements IMqttMessageListener
	 {
		 ...
		 SensorDataMessageListener(ResourceNameEnum resource, IDataMessageListener dataMsgListener)
		 {
			 this.resource = resource;
			 this.dataMsgListener = dataMsgListener;
			 _Logger.info("SensorDataMessageListener called");
		 }
	 ...
	 // Callback class for system performance data response messages subscription
	 private class SystemPerformanceDataMessageListener implements IMqttMessageListener
	 {
		 ...
		 public void messageArrived(String topic, MqttMessage message) throws Exception
		 {
			 try {
				 ...
				 // optionally, log a message indicating data was received
				 _Logger.info("Received SysPerfData response: " + systemPerformanceData.getCpuUtilization());
	 
				 if (this.dataMsgListener != null) {
					 this.dataMsgListener.handleSystemPerformanceMessage(resource, systemPerformanceData);
				 }
			 } catch (Exception e) {
				 _Logger.warning("Failed to convert message payload to SysPerfData.");
			 }
